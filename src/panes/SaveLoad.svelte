...
<Dialog open={$open} on:close={() => open.set(false)}>
  ...
  <div class="dialog">
    ...
    <footer>
      <button title="Add the songs and patterns from the data dump to what you already have" on:click={() => load(true)}>Append</button>
      <button title="Replace the songs and patterns with those from the data dump" on:click={() => load(false)}>Replace</button>
